...
describe("TagsModal", () => {
  ...
  describe("API", () => {
    it("should NOT call onApply select correct tag", () => {
      ...
      expect(
        wrapper.find("TagsModal").first().instance().state.selected[0]
      ).toMatchObject({
    ...
    it("should filter tags by", () => {
      ...
      const wrapper = mount(<TagsModal store={store} />);
      wrapper
        .find(".ins-c-conditional-filter input")
        .first()
        .simulate("change", {
          target: {
            value: "something",
      ...
      expect(
        wrapper.find("TagsModal").first().instance().state.filterTagsBy
      ).toBe("something");
    });
    ...
    it("should fetch next tags for system", () => {
      ...
      const actions = store.getActions();
      expect(actions[0]).toMatchObject({
        meta: { systemId: "something", tagsCount: 50 },
        type: "LOAD_TAGS_PENDING",
      });
    });
