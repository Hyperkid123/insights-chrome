  ...
  if (hasItems && items.length > 0) {
    ...
    return hosts
      .apiHostGetHostList(
        ...
        orderDirection,
        staleFilter,
        [...constructTags(tagFilters), ...(options.tags || [])],
        registeredWithFilter,
        undefined,
        {
          cancelToken: controller && controller.token,
...
export function getAllTags(
  search,
  { filters, pagination, ...options } = { pagination: {} }
) {
  const { tagFilters, staleFilter, registeredWithFilter } = filters
    ? filters.reduce(filtersReducer, defaultFilters)
    : defaultFilters;
