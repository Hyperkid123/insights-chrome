...
describe("getAllTags", () => {
  it("should generate get all tags call", async () => {
    mockTags
      .onGet(
        "/api/inventory/v1/tags?order_by=tag&order_how=ASC&per_page=10&page=1&staleness=fresh&staleness=stale&registered_with=insights"
      )
  ...
  describe("tagFilters", () => {
    ...
    it("should generate get all tags call with staleFilter", async () => {
      mockTags
        .onGet(
          "/api/inventory/v1/tags?order_by=tag&order_how=ASC&per_page=10&page=1&staleness=something&registered_with=insights"
        )
        .replyOnce(200, { test: "test" });
      const data = await getAllTags(undefined, {
        filters: [
    ...
    it("should generate get all tags call with registeredWithFilter", async () => {
      mockTags
        .onGet(
          "/api/inventory/v1/tags?order_by=tag&order_how=ASC&per_page=10&page=1&staleness=fresh&staleness=stale&registered_with=something"
        )
        .replyOnce(200, { test: "test" });
      const data = await getAllTags(undefined, {
        filters: [
  ...
  describe("pagination", () => {
    it("should generate get all tags call with perPage", async () => {
      mockTags
        .onGet(
          "/api/inventory/v1/tags?order_by=tag&order_how=ASC&per_page=50&page=1&staleness=fresh&staleness=stale&registered_with=insights"
        )
        .replyOnce(200, { test: "test" });
      const data = await getAllTags(undefined, {
        pagination: {
    ...
    it("should generate get all tags call with page", async () => {
      mockTags
        .onGet(
          "/api/inventory/v1/tags?order_by=tag&order_how=ASC&per_page=10&page=20&staleness=fresh&staleness=stale&registered_with=insights"
        )
        .replyOnce(200, { test: "test" });
      const data = await getAllTags(undefined, {
        pagination: {
