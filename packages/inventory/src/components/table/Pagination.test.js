...
describe("Pagination", () => {
  ...
  describe("render", () => {
    ...
    it("should render correctly with data and props", () => {
      ...
      const wrapper = render(
        <Pagination store={store} page={1} perPage={50} total={500} />
      );
      expect(toJson(wrapper)).toMatchSnapshot();
    ...
    it("should render correctly with data and isFull", () => {
      const store = mockStore(initialState);
      const wrapper = render(
        <Pagination store={store} page={1} perPage={50} total={500} isFull />
      );
      expect(toJson(wrapper)).toMatchSnapshot();
      expect(wrapper.find('button[disabled=""]').length).toBe(2);
    });
    ...
    it("should render correctly with hasItems and isLoaded false", () => {
      const store = mockStore(initialState);
      const wrapper = render(
        <Pagination store={store} hasItems isLoaded={false} />
      );
      expect(toJson(wrapper)).toMatchSnapshot();
    });
  });
  ...
  describe("API", () => {
    it("should call perPage change without onRefresh", () => {
      ...
      expect(wrapper.find("FooterPagination").instance().state).toMatchObject({
        page: 1,
      });
