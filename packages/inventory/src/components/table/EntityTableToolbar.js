...
import {
  tagsFilterState,
  tagsFilterReducer,
} from "@redhat-cloud-services/frontend-components/components/cjs/FilterHooks";
...
const EntityTableToolbar = ({
  ...
  const shouldReload =
    page && perPage && filters && (!hasItems || items) && loaded;
  ...
  const isFilterSelected = () => {
    return (
      textFilter.length > 0 ||
      flatMap(Object.values(selectedTags), (value) =>
    ...
    <Fragment>
      <PrimaryToolbar
        ...
        className={`ins-c-inventory__table--toolbar ${
          hasItems ? "ins-c-inventory__table--toolbar-has-items" : ""
        }`}
          ...
          loaded ? (
            {
              ...
              onSetPage: (_e, newPage) =>
                updateData({ page: newPage, per_page: perPage, filters }),
              onPerPageSelect: (_e, newPerPage) =>
                updateData({ page: 1, per_page: newPerPage, filters }),
            }
          ) : (
